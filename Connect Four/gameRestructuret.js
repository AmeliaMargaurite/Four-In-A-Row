...
function dropColumn(column) {
    ...
    for (i = 0; i < col.length; i++){
        console.log(i);
        if (col[i].filled === null) {
            col[i].filled = currentPlayer;
            id = col[i].id;
...
function dropDisc(id) {
    ...
    directions.forEach(direction => tester(direction, id)); // checks for matching discs using directions array
}
        ...
        for (a = 0; a < 2; a++) { // iterate through first level arrays in directions, directions are grouped either side of the id
            ...
            for (i = 0; i < 3; i++) { // iterate through second levels of arrays
                ...
                for (c = 0; c < board.length; c++){
                    // put into separate function to call
                    if (board[c].find(x => x.id === id + direction[1]).filled == currentPlayer) {
                        score.push(1);
